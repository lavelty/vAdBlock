...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function generateLabels(period, count) {
    ...
    if (period === '24h') {
      ...
      const days = [
        window.vAdBlockT('day_sun') || 'Paz',
        window.vAdBlockT('day_mon') || 'Pzt',
        window.vAdBlockT('day_tue') || 'Sal',
        window.vAdBlockT('day_wed') || 'Çar',
        window.vAdBlockT('day_thu') || 'Per',
        window.vAdBlockT('day_fri') || 'Cum',
        window.vAdBlockT('day_sat') || 'Cmt'
      ];
      ...
      const months = [
        window.vAdBlockT('month_jan') || 'Oca',
        window.vAdBlockT('month_feb') || 'Şub',
        window.vAdBlockT('month_mar') || 'Mar',
        window.vAdBlockT('month_apr') || 'Nis',
        window.vAdBlockT('month_may') || 'May',
        window.vAdBlockT('month_jun') || 'Haz',
        window.vAdBlockT('month_jul') || 'Tem',
        window.vAdBlockT('month_aug') || 'Ağu',
        window.vAdBlockT('month_sep') || 'Eyl',
        window.vAdBlockT('month_oct') || 'Eki',
        window.vAdBlockT('month_nov') || 'Kas',
        window.vAdBlockT('month_dec') || 'Ara'
      ];
  ...
  function drawChart(historyDataArray) {
    ...
    shownHitPoints.forEach((pt, idx) => {
      ...
      hit.addEventListener('mouseenter', () => {
        ...
        if (tooltip) {
          tooltip.textContent = pt.val.toLocaleString();
  ...
  function formatSkipped(sec) {
    ...
    const s = sec % 60;
    if (h > 0) return h + ` ${window.vAdBlockT('time_hours') || 'sa'} ` + m + ` ${window.vAdBlockT('time_minutes') || 'dk'}`;
    if (m > 0) return m + ` ${window.vAdBlockT('time_minutes') || 'dk'} ` + s + ` ${window.vAdBlockT('time_seconds') || 'sn'}`;
    return s + ` ${window.vAdBlockT('time_seconds') || 'sn'}`;
  }
  ...
  function updateStats() {
    chrome.runtime.sendMessage({ type: 'GET_STATS' }, (s) => {
      if (s && totalBlockedCount) {
        totalBlockedCount.textContent = (s.total || 0).toLocaleString();
        if (statAds) statAds.textContent = (s.ads || 0).toLocaleString();
        if (statTrackers) statTrackers.textContent = (s.trackers || 0).toLocaleString();
        if (statCdn) statCdn.textContent = (s.cdn || 0).toLocaleString();
        if (statOther) statOther.textContent = (s.other || 0).toLocaleString();
    ...
    chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
      if (tabs[0] && pageBlockedCount) {
        chrome.runtime.sendMessage({ type: 'GET_PAGE_STATS', tabId: tabs[0].id }, (pageBlocked) => {
          pageBlockedCount.textContent = (pageBlocked || 0).toLocaleString();
        });
      }
    });
  }
    ...
    btnReportIssue.addEventListener('click', () => {
      ...
      chrome.storage.local.get({ globalEnabled: true, protectionLevel: 'balanced' }, (r) => {
        ...
        const lines = [
          ...
          M('report_date') + ': ' + new Date().toLocaleString()
        ];
        ...
        if (tabs[0] && tabs[0].url.includes('youtube.com/watch')) {
          chrome.tabs.sendMessage(tabs[0].id, { action: 'openSponsorMenu' }, (resp) => {
            if (chrome.runtime.lastError) {
              showToast(window.vAdBlockT('toast_refresh_youtube') || 'YouTube sayfasını yenileyin.', true);
            } else {
              window.close();
            }
          });
        } else {
          showToast(window.vAdBlockT('toast_youtube_only') || 'Bu özellik sadece YouTube videolarında çalışır.', true);
        }
      });
    });
  }
  ...
  function initPromoSystem() {
    ...
    chrome.storage.local.get({ settings: {}, promo_discount_shown: false, installTime: Date.now() }, (res) => {
      ...
      const promos = [
        {
          id: 'rating',
          title: window.vAdBlockT('promo_rate_title') || '⭐ vAdBlock\'u beğendiniz mi?',
          desc: window.vAdBlockT('promo_rate_desc') || 'Geliştirmeye devam edebilmemiz için bize 5 yıldız vererek destek olabilirsiniz!',
          btn: window.vAdBlockT('promo_rate_btn') || 'Değerlendir'
        },
        {
          id: 'premium',
          title: window.vAdBlockT('promo_prem_title') || '🚀 vAdBlock Premium',
          desc: window.vAdBlockT('promo_prem_desc') || 'Premium abonelik alarak %100 gizlilik ve ekstra özelliklerden faydalanmak ister misiniz?',
          btn: window.vAdBlockT('promo_prem_btn') || 'İncele'
        }
      ];
      ...
      if (daysUsed >= 3 && !res.promo_discount_shown) {
        promos.push({
          id: 'discount',
          title: window.vAdBlockT('promo_disc_title') || '🎁 Sana Özel İndirim!',
          desc: (window.vAdBlockT('promo_disc_desc') || 'Uzun süredir bizimlesin! Premium üyelikte %50 indirim kazanmak için kodun: LAVE-') + Math.random().toString(36).substring(2,6).toUpperCase(),
          btn: window.vAdBlockT('promo_disc_btn') || 'Kodu Kullan'
        });
      }
      ...
      btnAction.onclick = () => {
        ...
        showToast(window.vAdBlockT('toast_redirecting') || 'Yönlendiriliyor...');
  ...
  document.addEventListener('vAdBlockLangLoaded', () => {
    updateStats();
    updateSkippedStat();
  });
  document.addEventListener('vadblock_i18n_ready', () => {
    updateStats();
    updateSkippedStat();
  });
});
